...
参考 spool 的 ACP 集成但独立设计，核心差异：

1. **直接使用官方包名** — `@agentclientprotocol/claude-agent-acp` 和 `@zed-industries/codex-acp`，不使用 spool 的 `acp-extension-{name}` 别名约定。
...
3. **精简 Client handler** — Thinklish 的查词场景不需要终端、文件系统等 agent 能力，Client handler 只实现 `sessionUpdate`（流式文本）和 `requestPermission`（自动批准），比 spool 的完整实现轻量得多。
4. **适配器解析器模式** — 每个 agent 适配器有独立的 `resolve` 函数，知道如何找到并启动自己的进程。Claude 适配器走 Node.js，Codex 适配器走原生二进制。这比 spool 的 extension/native/websocket/sdk 四模式单体类更清晰。
...
6. **不照搬 spool 实现** — 使用官方包、独立设计接口、精简 Client handler。
